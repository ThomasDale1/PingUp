...
const StoryViewer = ({viewStory, setViewStory}) => {
    ...
    const renderContent = () => {
        switch (viewStory.media_type) {
            ...
            case 'video':
                return(
                    <video controls autoPlay onEnded={handleClose} src={viewStory.media_url} className='max-h-screen'/>
                )
            case 'text':
                return(
  ...
  return (
    <div className='fixed inset-0 h-screen bg-black bg-opacity-90 z-110 flex items-center justify-center' style={{backgroundColor: viewStory.media_type === 'text' ? viewStory.background_color : '#000'}}>
        ...
        <button onClick={handleClose} className='absolute top-4 right-4 text-white text-3xl font-bold focus:outline-none'>
            <X className='cursor-pointer w-8 h-8 hover:scale-110 transition'/>
        </button>
